...
Barrel.calcFeeLevel()
*/
Barrel.prototype.fireUTXOs = function (utxos) {
    // recoil and load
    ...
    else global.log.log(`[Minigun] ${utxos.length} UTXO ammo fired in ${new Date().getTime() - startTime}ms, ${Math.floor(utxos.length / ((new Date().getTime() - startTime) / 1000))} TPS`)
...
Barrel.prototype.fireUTXO = function (utxo) {
    var tx = this.buildTX(utxo)
    if(tx && global.load){
...
Barrel.prototype.buildTX = function (utxo) {
    // 使用Change链，而非分裂
...
Barrel.prototype.loadFrom = function (privateKey, providedUTXOs) {
    ...
    return getUTXOs.then(utxos => {
        ...
        while (aggregateUtxos.length > 0) {
            ...
            global.log.log(`[Minigun] Recycling ${utxoschunk.length} utxo(s) into ${nRound} round(s) and ${nBullet} bullet(s)`)
            var tx = bsv.Transaction()
            tx.addData(["Stress Test", "[G] Heavy MachineGun!"])
            while (nRound-- > 0) tx.to(this.recycleAddr, FIRE_THESHOLD * MAX_OUTS_PER_TX)
            while (nBullet-- > 0) tx.to(barrelAddr, FIRE_THESHOLD)
